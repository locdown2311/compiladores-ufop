...
public class TypeContext <N,V> {

		private Hashtable<N, V> st;

		/**
		 * Default constructor.
		 */
	    public TypeContext(){
		   st = new Hashtable<N, V>();
	    }
	    ...
	    public String toStringf(String id){
	    	String s = id + "------ Type Context ----- \n";
	    	for(Entry<N,V> e : st.entrySet()){
	    		s += id + " " + e.getKey().toString() + " |-> " + e.getValue().toString() + "\n";
	    	}
	    	return s;
	    }

		public Set<Entry<N,V>> getEntrySet(){
	         return st.entrySet();
	    }


		public void add(N n, V v){ st.put(n,v); }
		 ...
		 */
		public V get(N n){return st.get(n);}


		public boolean contains(N s){ return st.containsKey(s);}
